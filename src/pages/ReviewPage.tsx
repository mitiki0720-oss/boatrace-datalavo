...
import { loadBoatJohnsonPredictionRecords } from "../lib/boatJohnsonPredictionStorage";
...
import type { BoatJohnsonPredictionPayload, BoatJohnsonPredictionRecord, BoatRaceItem, BoatTodayFeed } from "../lib/boatraceTypes";
...
const PUBLIC_JOHNSON_PREDICTIONS_PATH = "data/boatrace/johnson-predictions.generated.json";

async function loadPublicJohnsonPredictionRecords(): Promise<BoatJohnsonPredictionRecord[]> {
	try {
		const response = await fetch(withBasePath(PUBLIC_JOHNSON_PREDICTIONS_PATH), { cache: "no-store" });
		if (!response.ok) return [];
		const payload = await response.json() as Partial<BoatJohnsonPredictionPayload>;
...
type ReviewExportAction = "prediction-copy" | "prediction-txt" | "result-copy" | "result-txt";
type ReviewExportFeedback = "idle" | "success" | "error";

const INITIAL_EXPORT_FEEDBACK: Record<ReviewExportAction, ReviewExportFeedback> = {
	"prediction-copy": "idle",
	"prediction-txt": "idle",
	"result-copy": "idle",
	"result-txt": "idle",
};

function isReviewExportTextAvailable(text: string): boolean {
	const normalized = text.trim();
	return Boolean(normalized) && !/^(?:会場を選択してください|予想ファイル未登録|結果ファイル未登録)$/.test(normalized);
}

function getReviewExportButtonLabel(feedback: ReviewExportFeedback, idleLabel: "コピー" | "TXT"): string {
	if (feedback === "success") return idleLabel === "コピー" ? "✓ コピー済み" : "✓ 保存しました";
	if (feedback === "error") return idleLabel === "コピー" ? "コピー失敗" : "保存失敗";
	return idleLabel;
}
...
export function ReviewPage() {
	...
	const [johnsonPayload, setJohnsonPayload] = useState(() => loadBoatJohnsonPredictionRecords());
	const [publicJohnsonRecords, setPublicJohnsonRecords] = useState<BoatJohnsonPredictionRecord[]>([]);
	...
	const [exportFeedback, setExportFeedback] = useState(INITIAL_EXPORT_FEEDBACK);
	const selectedVenueDetailRef = useRef<HTMLElement | null>(null);
	const exportFeedbackTimers = useRef<Partial<Record<ReviewExportAction, number>>>({});

	useEffect(() => () => {
		Object.values(exportFeedbackTimers.current).forEach((timer) => window.clearTimeout(timer));
	}, []);

	useEffect(() => {
		Object.values(exportFeedbackTimers.current).forEach((timer) => window.clearTimeout(timer));
		exportFeedbackTimers.current = {};
		setExportFeedback(INITIAL_EXPORT_FEEDBACK);
	}, [selectedVenueKey]);
	...
	useEffect(() => {
		...
		]).then(([feed, index, venueExtras, publicJohnson]) => {
			if (!active) return;
			setTodayFeed(feed);
			setArchiveIndex(index);
			setVenueExtrasFeed(venueExtras);
			setPublicJohnsonRecords(publicJohnson);
		});
		return () => {
			active = false;
		};
	...
	const predictionRecords = useMemo(() => normalizeBoatPredictionRecordList([
		...Object.values(predictionPayload),
		...Object.values(johnsonPayload),
		...publicJohnsonRecords,
	]), [johnsonPayload, predictionPayload, publicJohnsonRecords]);
	...
	const predictionExportAvailable = isReviewExportTextAvailable(predictionText);
	const resultExportAvailable = isReviewExportTextAvailable(resultText);
	const modeLabel = mode === "archive" ? "ARCHIVE FILE" : selectedDate === operationalToday ? "TODAY LIVE" : "LIVE";
	const sourceLabel = mode === "archive"
		? "archive txt"
		: "official result + public/local prediction";
	...
	const refreshLiveData = () => {
		...
		setJohnsonPayload(loadBoatJohnsonPredictionRecords());
		...
		void loadPublicJohnsonPredictionRecords().then(setPublicJohnsonRecords);
		setStatusMessage("今日・昨日のlocalStorageを優先しつつ、必要なら archive も再読み込みしました");
	};

	const showExportFeedback = (action: ReviewExportAction, feedback: Exclude<ReviewExportFeedback, "idle">) => {
		const currentTimer = exportFeedbackTimers.current[action];
		if (currentTimer !== undefined) window.clearTimeout(currentTimer);
		setExportFeedback((current) => ({ ...current, [action]: feedback }));
		exportFeedbackTimers.current[action] = window.setTimeout(() => {
			setExportFeedback((current) => ({ ...current, [action]: "idle" }));
			delete exportFeedbackTimers.current[action];
		}, 1800);
	};

	const handleVenueSelect = (venueKey: string) => {
		setSelectedVenueKey(venueKey);
		setStatusMessage("");
		window.requestAnimationFrame(() => {
			selectedVenueDetailRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
		});
	};

	const handleCopy = async (action: ReviewExportAction, text: string, label: string) => {
		if (!isReviewExportTextAvailable(text)) return;
		const ok = await copyText(text);
		showExportFeedback(action, ok ? "success" : "error");
		setStatusMessage(ok ? `${label}をコピーしました` : `${label}のコピーに失敗しました`);
	};

	const handleDownload = (action: ReviewExportAction, filename: string, text: string, label: string) => {
		if (!isReviewExportTextAvailable(text)) return;
		const ok = downloadText(filename, text);
		showExportFeedback(action, ok ? "success" : "error");
		setStatusMessage(ok ? `${label}を保存しました` : `${label}の保存に失敗しました`);
	};
			...
			<div className="review-page-root">
				<main className="boat-review-workbench">
					...
					<section className="boat-review-panel boat-review-venues-panel">
						...
						{groups.length > 0 ? <div className="review-venue-grid">
							{groups.map((group) => {
								...
								return <button key={group.key} type="button" className="boat-review-venue-card" data-selected={isSelected} aria-pressed={isSelected} onClick={() => handleVenueSelect(group.key)}>
									<div className="boat-review-venue-head"><div><span>{formatVenueCardDate(group.date)}</span><strong>{group.venueName}</strong><small>{group.title || `${group.venueName} 出走表一覧`}</small></div><div>{isSelected ? <span className="boat-review-selected-badge">選択中</span> : null}<span>{getVenueStageLabel(group)}</span>{sessionLabel ? <small>{sessionLabel}</small> : null}</div></div>
					...
					{selectedGroup && selectedMetrics ? <>
						<section ref={selectedVenueDetailRef} className="boat-review-panel boat-review-detail">
							<div className="boat-review-section-heading">
						...
						<section className="boat-review-panel boat-review-copy-material">
							...
							<div className="boat-review-copy-list">
								<article className="boat-review-copy-card"><div><span>PREDICTION COPY</span><strong>予想まとめをコピー <em>({selectedMetrics.predictionRaceCount}/{selectedMetrics.targetRaceCount})</em></strong><small>{selectedMetrics.missingPredictionRaceNos.length ? `未入力: ${formatMissingRaceNos(selectedMetrics.missingPredictionRaceNos)}` : "全Rあり"}</small></div><div><button type="button" data-feedback={exportFeedback["prediction-copy"]} disabled={!predictionExportAvailable} onClick={() => void handleCopy("prediction-copy", predictionText, "予想まとめ")}>{getReviewExportButtonLabel(exportFeedback["prediction-copy"], "コピー")}</button><button type="button" data-feedback={exportFeedback["prediction-txt"]} disabled={!predictionExportAvailable} onClick={() => handleDownload("prediction-txt", getReviewFileName(selectedGroup.date, selectedGroup.venueSlug, "predictions"), predictionText, "予想TXT")}>{getReviewExportButtonLabel(exportFeedback["prediction-txt"], "TXT")}</button></div></article>
								<article className="boat-review-copy-card"><div><span>RESULT COPY</span><strong>結果まとめをコピー <em>({selectedMetrics.officialResultCount}/{selectedMetrics.targetRaceCount})</em></strong><small>{selectedMetrics.missingResultRaceNos.length ? `未取得: ${formatMissingRaceNos(selectedMetrics.missingResultRaceNos)}` : "全Rあり"}</small></div><div><button type="button" data-feedback={exportFeedback["result-copy"]} disabled={!resultExportAvailable} onClick={() => void handleCopy("result-copy", resultText, "結果まとめ")}>{getReviewExportButtonLabel(exportFeedback["result-copy"], "コピー")}</button><button type="button" data-feedback={exportFeedback["result-txt"]} disabled={!resultExportAvailable} onClick={() => handleDownload("result-txt", getReviewFileName(selectedGroup.date, selectedGroup.venueSlug, "results"), resultText, "結果TXT")}>{getReviewExportButtonLabel(exportFeedback["result-txt"], "TXT")}</button></div></article>
							</div>
