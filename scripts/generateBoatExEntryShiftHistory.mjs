...
for (const record of records) {
	...
	const venue = venues.get(record.venueCode) ?? { venueCode: record.venueCode, venueName: record.venueName, raceCount: 0, frameToCourseCounts: {}, frameNariRaceCount: 0, entryShiftRaceCount: 0, lane1InsideCount: 0, byRaceBand: {} };
	venues.set(record.venueCode, venue); venue.raceCount += 1;
	const shift = entries.some((entry) => asText(entry.lane) && asText(entry.course) && asText(entry.lane) !== asText(entry.course));
	if (shift) venue.entryShiftRaceCount += 1; else venue.frameNariRaceCount += 1;
	if (entries.some((entry) => asText(entry.lane) === "1" && asText(entry.course) === "1")) venue.lane1InsideCount += 1;
	for (const entry of entries) { const frame = asText(entry.lane); const course = asText(entry.course); if (frame && course) { venue.frameToCourseCounts[frame] ??= {}; increment(venue.frameToCourseCounts[frame], course); } }
