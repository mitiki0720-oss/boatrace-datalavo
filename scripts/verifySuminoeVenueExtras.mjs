...
const tempRoot = path.join(projectRoot, ".tmp", "suminoe-verification");
...
async function main() {
	...
	console.log(`[verify-suminoe] requestedTargetDate=${targetDate}`);
	console.log("[verify-suminoe] production JSON will not be modified");
	...
	if (!venue) {
		console.log("[verify-suminoe] skipped (not held on target date)");
		return;
	}

	assert.equal(venue.integrationMode, "venue-official");
	assert.equal(venue.officialVenueExtrasSupported, true);
	assert.equal(venue.dedicatedParserKey, "suminoe");
	assert.equal((venue.races ?? []).length, 12, "held Suminoe should expose 12 races");
	...
	assert.ok(race.entryTable.every((row) => row.registrationNo && row.playerName && row.motorNo), "entry rows should include racer and motor data");
	assert.ok(race.entryTable.some((row) => row.boatNo && row.boatSecondRate), "entry rows should include joined boat data from precheck");
	assert.ok(venue.waterSurfaceInfo && venue.waterSurfaceInfo.waterQuality, "water surface info should be parsed");
	const oddsCount = race.suminoeTrifectaOdds?.length ?? 0;
	assert.ok(oddsCount === 0 || oddsCount === 120, "trifecta odds must be empty or complete 120 rows");

	console.log(`[verify-suminoe] status=${venue.status} integrationMode=${venue.integrationMode}`);
	console.log(`[verify-suminoe] races=${venue.races.length} entry=${race.entryTable.length} motor=${race.motorSummary.length}`);
	console.log(`[verify-suminoe] before=${race.beforeInfo?.length ?? 0} start=${race.startExhibition?.length ?? 0} original=${race.originalExhibition?.length ?? 0}`);
	console.log(`[verify-suminoe] waterSurface=${venue.sourceStatus?.waterSurfaceInfo} precheck=${venue.sourceStatus?.motorLotteryAndPrecheck}`);
	console.log(`[verify-suminoe] trifectaOdds=${oddsCount} classification=${oddsCount === 120 ? "complete" : race.sourceStatus?.trifectaOdds}`);
	console.log(`[verify-suminoe] sourceStatus=${JSON.stringify(race.sourceStatus)}`);
	console.log("[verify-suminoe] completed");
}

main().catch((error) => {
	console.error(`[verify-suminoe] ${error instanceof Error ? error.message : String(error)}`);
	process.exit(1);
});
